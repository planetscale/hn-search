...
async function main() {
  ...
  const client = new Client({ ...pgConnectionConfig(url), statement_timeout: 0, query_timeout: 0 })
  ...
  try {
    ...
    console.log('› CREATE EXTENSION tin')
    await client.query('CREATE EXTENSION IF NOT EXISTS tin')
    for (const file of ['drizzle/0001_items.sql', 'drizzle/0002_sync_state.sql']) {
      console.log(`› ${file}`)
      await client.query(await readFile(path.join(process.cwd(), file), 'utf8'))
    }
    const { rows } = await client.query<{ extname: string }>(`SELECT extname FROM pg_extension WHERE extname = 'tin'`)
