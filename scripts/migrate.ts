...
async function main() {
  ...
  try {
    ...
    for (const ext of ['lakebase_text', 'pg_trgm', 'pg_prewarm']) {
    ...
    const { rows } = await client.query<{ extname: string }>(`SELECT extname FROM pg_extension WHERE extname IN ('lakebase_text', 'pg_trgm', 'pg_prewarm') ORDER BY 1`)
